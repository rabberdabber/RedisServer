...
int main(int argc, char *argv[])
{
    ...
    buf_t dyn_buf_arr;
    char buf[BUFSIZE];
    buf_t *dyn_buf = &dyn_buf_arr;
    dyn_buf->buf = buf;
    dyn_buf->len = BUFSIZE;

    char *port, *redis_port, *read_addr;
    size_t cli_ret, serv_ret, content_len = 0;
    rio_t rio;
    ...
    for (;;)
    {
        ...
        if ((pid = fork()) >= 0)
        {
            if (!pid)
            {
                ...
                /*-----------------------------*/
                /* communicate with ab client  */
                /*-----------------------------*/

                vector<string> redis_str;
                vector<string> res;
                size_t temp, count = 0;
                bool finish = false;
                string redis_out;
                ...
                if ((redisfd = open_clientfd(argv[2], atoi(redis_port))) == -1)
                {
                    exit(EXIT_FAILURE);
                }
